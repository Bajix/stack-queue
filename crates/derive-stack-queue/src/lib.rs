...
#[proc_macro_derive(LocalSliceQueue, attributes(local_queue))]
pub fn derive_slice_queue(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
  let input = parse_macro_input!(input as DeriveInput);
  let ident = &input.ident;
  ...
  let expanded = quote! {
    ...
    #[cfg(not(loom))]
    impl stack_queue::LocalSliceQueue<#buffer_size> for #ident {
      fn queue() -> &'static std::thread::LocalKey<stack_queue::StackQueue<std::cell::UnsafeCell<std::mem::MaybeUninit<<#ident as stack_queue::SliceQueue>::Task>>, #buffer_size>> {
        thread_local! {
          static QUEUE: stack_queue::StackQueue<std::cell::UnsafeCell<std::mem::MaybeUninit<<#ident as stack_queue::SliceQueue>::Task>>, #buffer_size> = stack_queue::StackQueue::default();
        }
    ...
    #[cfg(loom)]
    impl stack_queue::LocalSliceQueue<#buffer_size> for #ident {
      fn queue() -> &'static stack_queue::loom::thread::LocalKey<stack_queue::StackQueue<loom::cell::UnsafeCell<std::mem::MaybeUninit<<#ident as stack_queue::SliceQueue>::Task>>, #buffer_size>> {
        stack_queue::loom::thread_local! {
          static QUEUE: stack_queue::StackQueue<loom::cell::UnsafeCell<std::mem::MaybeUninit<<#ident as stack_queue::SliceQueue>::Task>>, #buffer_size> = stack_queue::StackQueue::default();
        }
